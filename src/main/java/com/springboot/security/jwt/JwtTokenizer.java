...
public class JwtTokenizer {
    ...
    public String generateAccessToken(Map<String, Object> claims,
        ...
        valueOperations.set((String) claims.get("membername"), accessToken, accessTokenExpirationMinutes, TimeUnit.MINUTES);
        return accessToken;
    }
    ...
    public boolean deleteRegisterToken(String membername) {
        return Optional.ofNullable(redisTemplate.hasKey(membername))
                .filter(Boolean::booleanValue) // 키가 존재할 때만 진행
                .map(hasKey -> {
                    String accessToken = (String) redisTemplate.opsForValue().get(membername);
                    redisTemplate.delete(accessToken);
                    redisTemplate.delete(membername);
                    return true;
                })
