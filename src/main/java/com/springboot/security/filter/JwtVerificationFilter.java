...
public class JwtVerificationFilter extends OncePerRequestFilter {
   ...
   private final JwtAuthorityUtils authorityUtils;
    ...
    private void setAuthenticationToContext(Map<String, Object> claims){
        ...
        Authentication authentication = new membernamePasswordAuthenticationToken(
                membername, null, authorities);
        SecurityContextHolder.getContext().setAuthentication(authentication);
    }

    // Redis에서 토큰을 검증하는 메서드 추가
    private void isTokenValidInRedis(Map<String, Object> claims) {
        String membername = Optional.ofNullable((String) claims.get("membername"))
                .orElseThrow(() -> new NullPointerException("membername is null"));

        // Redis에 해당 키(membername)가 존재하는지 확인
        Boolean hasKey = redisTemplate.hasKey(membername);

        // 키가 존재하지 않거나 null일 경우 예외를 던짐
        if (Boolean.FALSE.equals(hasKey)) {
            throw new IllegalStateException("Redis key does not exist for membername: " + membername);
        }
    }
}
